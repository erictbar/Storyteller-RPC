...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("Storyteller Discord RPC Installer/Updater");
    ...
    if latest_installer_version != current_installer_version {
        ...
        println!("Please download the latest version from https://github.com/0xGingi/storyteller-discord-rpc/releases/tag/{}", latest_installer_version);
    }
    ...
    println!("Latest version: {}", latest_version);    let (download_url, install_path) = if cfg!(target_os = "windows") {
        (
            format!("https://github.com/0xGingi/storyteller-discord-rpc/releases/download/{}/storyteller-discord-rpc.exe", latest_version),
            PathBuf::from(std::env::var("LOCALAPPDATA")?).join("StorytellerDiscordRPC").join("storyteller-discord-rpc.exe")
        )
    } else if cfg!(target_os = "macos") {
        (
            format!("https://github.com/0xGingi/storyteller-discord-rpc/releases/download/{}/storyteller-discord-rpc-macos-arm64", latest_version),
            PathBuf::from(std::env::var("HOME")?).join(".local").join("bin").join("storyteller-discord-rpc")
        )
    } else {
        (
            format!("https://github.com/0xGingi/storyteller-discord-rpc/releases/download/{}/storyteller-discord-rpc-linux-x64", latest_version),
            PathBuf::from(std::env::var("HOME")?).join(".local").join("bin").join("storyteller-discord-rpc")
        )
    };
...
async fn get_latest_version(client: &Client) -> Result<String, Box<dyn std::error::Error>> {
    ...
    let resp = client.get(url)
        .header("User-Agent", "Storyteller-Discord-RPC-Installer")
        .send()
        .await?;
...
fn generate_config() -> Result<serde_json::Value, io::Error> {
    println!("Please enter the following information:");    let storyteller_url = prompt("Storyteller URL (Don't forget to include port if not reverse proxying)")?;
    let storyteller_username = prompt("Storyteller Username")?;
    let storyteller_password = prompt("Storyteller Password")?;
    ...
    let show_progress = prompt_with_default("Show reading progress instead of book info? (yes/no)", "no")?
        .to_lowercase()
        .starts_with('y');
    
    println!("\n--- Cover Art Options ---");
    let use_storyteller_cover = prompt_with_default("Use cover art from your Storyteller server instead of external providers? (yes/no)", "yes")?
        .to_lowercase()
        .starts_with('y');
    
    let imgur_client_id = if use_storyteller_cover {
        println!("To use Storyteller covers, you need an Imgur Client ID to host the images for Discord.");
...
fn create_windows_service(install_path: &PathBuf) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let task_name = "StorytellerDiscordRPC";
...
<dict>
    <key>Label</key>
    <string>com.storyteller.discord-rpc</string>
    ...
    <string>/tmp/storyteller-discord-rpc.log</string>
...
"#,
    ...
    let plist_path = launch_agents_dir.join("com.storyteller.discord-rpc.plist");
...
fn create_linux_service(install_path: &PathBuf) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let service_content = format!(
        r#"[Unit]
Description=Storyteller Discord RPC
...
"#,
    ...
    let service_path = config_dir.join("storyteller-discord-rpc.service");
    ...
    Command::new("systemctl")
        ...
        .status()?;
    Command::new("systemctl")
        .args(&["--user", "enable", "--now", "storyteller-discord-rpc"])
        .status()?;
...
fn stop_service() -> Result<(), Box<dyn std::error::Error>> {
    #[cfg(target_os = "windows")]
    {
        Command::new("schtasks")
            .args(&["/End", "/TN", "StorytellerDiscordRPC"])
            .status()?;
    }
    ...
    {
        Command::new("launchctl")
            .args(&["unload", &format!("{}/Library/LaunchAgents/com.storyteller.discord-rpc.plist", std::env::var("HOME")?)])
            .status()?;
    }
    ...
    {
        Command::new("systemctl")
            .args(&["--user", "stop", "storyteller-discord-rpc"])
            .status()?;
    }
...
fn start_service() -> Result<(), Box<dyn std::error::Error>> {
    ...
    {
        Command::new("schtasks")
            .args(&["/Run", "/TN", "StorytellerDiscordRPC"])
            .status()?;
    }
    ...
    {
        Command::new("launchctl")
            .args(&["load", "-w", &format!("{}/Library/LaunchAgents/com.storyteller.discord-rpc.plist", std::env::var("HOME")?)])
            .status()?;
    }
    ...
    {
        Command::new("systemctl")
            .args(&["--user", "start", "storyteller-discord-rpc"])
            .status()?;
    }
...
async fn get_latest_installer_version(client: &Client) -> Result<String, Box<dyn std::error::Error>> {
    let url = "https://api.github.com/repos/0xGingi/storyteller-discord-rpc/releases";
    let resp = client.get(url)
        .header("User-Agent", "Storyteller-Discord-RPC-Installer")
        .send()
        .await?;
